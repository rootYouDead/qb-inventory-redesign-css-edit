const InventoryContainer = Vue.createApp({
    ...
    methods: {
        ...
        openInventory(data) {
            ...
            // Update player info
            if (data.playerinfo) {
                this.playerName = data.playerinfo.name || "";
                this.playerID = data.playerinfo.id || "";
                this.playerCash = data.playerinfo.cash || 0;
            }
            if (data.inventory) {
        ...
        generateTooltipContent(item) {
            ...
            let content = `<div class="custom-tooltip" style="background-color: var(--surface-container-high); padding: 12px; border-radius: var(--radius-small); box-shadow: var(--elevation-2); color: var(--on-surface);">`;
            content += `<div class="tooltip-header" style="font-weight: var(--font-weight-bold); margin-bottom: 5px;">${item.label}</div><hr class="tooltip-divider">`;
            ...
            if (item.info && Object.keys(item.info).length > 0) {
                for (const [key, value] of Object.entries(item.info)) {
                    if (key !== "description") {
                        ...
                        content += `<div class="tooltip-info" style="margin-bottom: 3px;"><span class="tooltip-info-key" style="font-weight: var(--font-weight-medium);">${this.formatKey(key)}:</span> ${valueStr}</div>`;
                    }
                }
            }

            content += `<div class="tooltip-description" style="margin-top: 5px; font-size: 0.9em;">${description}</div>`;
            content += `<div class="tooltip-weight" style="margin-top: 5px; font-size: 0.9em;"><i class="fas fa-weight-hanging"></i> ${item.weight !== undefined && item.weight !== null ? (item.weight / 1000).toFixed(1) : "N/A"}kg</div>`;
